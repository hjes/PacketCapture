...
//            }
            ...
            if (multiPacketFilter != null)//如果满足过滤要求则对该包进行处理
            {
                if (multiPacketFilter.packetFilter(packet)) {
                    PcapPacket tempPacket = new PcapPacket(packet);
                    processorAndObserveThread.process(new PacketWrapper(tempPacket));
                }
            } else {//不设置过滤器
                PcapPacket tempPacket = new PcapPacket(packet);
                processorAndObserveThread.process(new PacketWrapper(tempPacket));
...
//                tempPacket.transferHeaderAndDataFrom(pcapHeader,)
            }
